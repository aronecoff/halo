...
const DIMENSION_LABELS = [
  { key: "Attachment Compatibility", short: "Attachment", desc: "Did the bond feel natural?" },
  { key: "Communication Resonance", short: "Communication", desc: "Could you hear each other?" },
  { key: "Values Alignment", short: "Values", desc: "Do you care about the same things?" },
  { key: "Emotional Intelligence", short: "Emotional IQ", desc: "Were they emotionally aware?" },
  { key: "Growth Trajectory", short: "Growth", desc: "Are they headed somewhere?" },
  { key: "Intent Alignment", short: "Intent", desc: "Were they there for the right reasons?" },
];
...
function buildDebriefFromMatch(matchData: any) {
  ...
  if (sharedTraits.length > 0) {
    ...
    sharedTraits.forEach((trait: string, i: number) => {
      ...
      items.push({ label, after: baseScore, change: `+${improvement.toFixed(2)}` });
    });
  }
...
export function PostMeetingScreen({ onRestart, matchData, matchId }: PostMeetingScreenProps) {
  ...
  const handleSelect = (id: string) => {
    setSelected(id);
    // Show dimension rating step before submitting
    setTimeout(() => setShowDimensions(true), 400);
  };
      ...
      try {
        await fetch("/api/feedback", {
          ...
          body: JSON.stringify({
            matchId,
            response: selected,
            dimensionRatings: Object.keys(dimensionRatings).length > 0 ? dimensionRatings : undefined,
          }),
        });
  ...
  return (
    <ScreenWrap k="postMeeting">
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px 24px" }}>
        ...
        {/* Step 1: Overall response */}
        {!showDimensions && !showDebrief && (
        ...
        {/* Step 2: Dimension ratings — trains the learning engine */}
        {showDimensions && !showDebrief && (
          <div className="fade-up" style={{ marginTop: 8 }}>
            <Label style={{ marginBottom: 6, fontSize: 10, letterSpacing: 2.5, color: "#A78BFA" }}>RATE EACH DIMENSION</Label>
            <p style={{ fontSize: 11, color: "#6B7280", marginBottom: 16 }}>
              This teaches IRIS what matters most to you.
            </p>
            {DIMENSION_LABELS.map((dim, i) => {
              const val = dimensionRatings[dim.key] ?? 0.5;
              return (
                <div key={dim.key} className={`fade-up fade-up-${i + 1}`} style={{ marginBottom: 16 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontSize: 12, color: "#D1D5DB", fontWeight: 600 }}>{dim.short}</span>
                    <span style={{ fontSize: 11, color: "#6B7280" }}>{dim.desc}</span>
                  </div>
                  <div style={{ position: "relative", height: 28, display: "flex", alignItems: "center" }}>
                    <input
                      ...
                      onChange={e => handleDimensionChange(dim.key, parseInt(e.target.value) / 100)}
                      style={{
                        width: "100%",
                        height: 4,
                        appearance: "none",
                        WebkitAppearance: "none",
                        background: `linear-gradient(to right, #7C3AED ${val * 100}%, rgba(255,255,255,0.06) ${val * 100}%)`,
                        borderRadius: 2,
                        outline: "none",
                        cursor: "pointer",
                      }}
            ...
            <div style={{ marginTop: 8 }}>
              <PrimaryButton onClick={handleSubmitFeedback}>SUBMIT FEEDBACK</PrimaryButton>
            </div>
            <button
